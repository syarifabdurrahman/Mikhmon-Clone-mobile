...
import 'widgets/voucher_qr_fullscreen.dart';
import 'widgets/print_preview_dialog.dart';
import 'widgets/voucher_templates_dialog.dart';
...
class _VouchersListScreenState extends ConsumerState<VouchersListScreen> {
  ...
  void _showFullscreenQr(Voucher voucher) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => VoucherQrFullscreen(voucher: voucher),
      ),
    );
  }
  ...
  void _showVoucherActions(Voucher voucher) {
    showModalBottomSheet(
      ...
      builder: (context) => VoucherInfoSheet(
        voucher: voucher,
        onCopy: () {
          Navigator.pop(context);
          _copyVoucherToClipboard(voucher);
        },
        onShare: () {
          Navigator.pop(context);
          _shareVoucher(voucher);
        },
        onPrint: () {
          Navigator.pop(context);
          _printSingleVoucher(voucher);
        },
      ),
    );
  }

  void _copyVoucherToClipboard(Voucher voucher) {
    final text = 'Username: ${voucher.username}\nPassword: ${voucher.password}';
    // Clipboard.setData requires clipboard_handler package
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
  ...
  void _shareVoucher(Voucher voucher) {
...
${voucher.isExpired ? '⚠️ Expired' : '✅ Active'} - Generated by ΩMMON''';
  ...
  void _printSingleVoucher(Voucher voucher) {
    showDialog(
      context: context,
      builder: (context) => PrintPreviewDialog(
        vouchers: [voucher],
        onPrint: (vouchers) async {
          final template = ref.read(voucherTemplateProvider);
          await VoucherPrinter.printBulkVouchers(context, vouchers,
              template: template);
        },
      ),
    );
  ...
  void _showTemplatesDialog() {
    ...
    profilesAsync.when(
      data: (profiles) {
        ...
        showDialog(
          context: context,
          builder: (context) => VoucherTemplatesDialog(
            onTemplateSelected: (template) {
              showDialog(
                context: context,
                builder: (context) => TemplateGenerateDialog(
                  template: template,
                  profiles:
                      profileNames.isNotEmpty ? profileNames : ['default'],
                  onGenerate: (settings) {
                    // Navigate to generation with settings
                    context.push('/main/users/generate', extra: {
                      'template': template,
                      'settings': settings,
                    });
                  },
